...
def draw():
    global state, rsize, players, b1x, b1y, b2x, b2y, sbx, sby, sbw, tx, turn, rnd, nbx, nby, nbw, nbh, buttonx, buttony, buttonw, buttonh
    background(150)
    text(state,5,5)
    if state == 0:
        #minus button
    ...
    if state == 1:
        ...
        fill(0)
        text('Board',buttonx+buttonw/2,buttony+buttonh/2)
        if mouseX >= buttonx and mouseX < buttonx+buttonw and mouseY >= buttony and mouseY < buttony+buttonh: #draw the radiation zone only if the mouse is hovering over the button
            draw_radzone(radzone)
        elif radleft == 2 and rnd > 3 and radzone <= boardsize//2: #if the radiation zone is not being drawn, then do display the next turn indicator!
            fill(255,63,63)
            text('MOVE THE RADIATION ZONE!',13*radsize+200,-1*radsize+200)
